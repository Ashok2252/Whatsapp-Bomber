...
def startexe():
    ...
    msg = str(msgg.get())
    num = nooo.get()
    driver = webdriver.Chrome('C:/Users/User/AppData/Local/Temp/Rar$EXa24628.256/chromedriver.exe')
...
#GUI
win=tk.Tk()
win.title("Whatsapp Bomber by ASHOK ")
win.geometry("600x600")
x=Label(win,text="YOU HAVE 30 SECONDS TO SCAN AND LOGIN").place(x=140,y=10)
x=Label(win,text="Enter name").place(x=150,y=100)
name=StringVar()
namebox=Entry(win,textvariable=name).place(x=250,y=100)

x=Label(win,text="Enter Message").place(x=150,y=200)
